        ...
        builder: (context, constraints) {
          ...
          if (constraints.maxWidth > 100) {
            return Row(
              children: [
                // Left side with logo
                Expanded(child: _buildGradientLogoSection()),
                // Right side with login form
                Expanded(child: _buildLoginFormSection(context)),
              ],
            );
          } else {
  ...
  Widget _buildLoginFormSection(BuildContext context, {bool isMobile = false}) {
    return Padding(
      padding: EdgeInsets.symmetric(
        horizontal: isMobile ? 30.0 : 60.0,
        vertical: isMobile ? 40.0 : 0.0,
      ),
      child: Center(
        child: SingleChildScrollView(
          // Added for scrollability on smaller screens if content overflows
          child: Form(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                      return AppStrings.moodleUrlEmptyError;
                    }
                    // Basic URL validation
                    if (!value.startsWith('http://') &&
                        !value.startsWith('https://')) {
                      return AppStrings.moodleUrlValidityError;
                    }
                    return null;
                  },
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _isLoadingUsernamePass
                        ? null
                    ...
                    child:
                        _isLoadingUsernamePass // Show loading indicator based on username/pass loading state
                ...
                if (kIsWeb) // Only for web
                  Align(
                    // Center the button if it's smaller than full width
                    alignment: Alignment.center,
                    child: SizedBox(
                      width: 250, // Google's button has a fixed width
                      height: 45,
                      child: web
                          .renderButton(), // This renders the actual Google button
                    ),
                  )
                ...
                SizedBox(height: 20),
                Text(
                  AppStrings.termsAndPrivacyText,
