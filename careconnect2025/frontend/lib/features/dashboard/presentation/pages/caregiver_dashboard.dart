...
Future<void> fetchPatients() async {
  ...
  Widget buildVitalsSummary(List<Map<String, dynamic>> vitals) {
    ...
    num minVal(List<num> values) => values.isEmpty ? 0 : values.reduce((a, b) => a < b ? a : b);
    num maxVal(List<num> values) => values.isEmpty ? 0 : values.reduce((a, b) => a > b ? a : b);
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('Vitals Summary (7 days)', style: TextStyle(fontWeight: FontWeight.bold)),
            Text('Heart Rate: avg ${avg(heartRates).toStringAsFixed(1)} bpm, '
                'min ${minVal(heartRates)}, max ${maxVal(heartRates)}'),
            Text('SpO₂: avg ${avg(spo2s).toStringAsFixed(1)}%, '
                'min ${minVal(spo2s)}, max ${maxVal(spo2s)}'),
            Text('BP (Systolic): avg ${avg(systolics).toStringAsFixed(1)}, '
                'min ${minVal(systolics)}, max ${maxVal(systolics)} mmHg'),
            Text('BP (Diastolic): avg ${avg(diastolics).toStringAsFixed(1)}, '
                'min ${minVal(diastolics)}, max ${maxVal(diastolics)} mmHg'),
            Text('Weight: avg ${avg(weights).toStringAsFixed(1)} lbs, '
