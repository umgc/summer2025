...
class AuthService with ChangeNotifier {
  ...
  // IMPORTANT: Call this initialization method early
  ...
  // Corrected parameter type to GoogleSignInAuthenticationEvent
  Future<void> _handleAuthenticationEvent(
    GoogleSignInAuthenticationEvent event,
  ) async {
    GoogleSignInAccount? googleUser;
    // Use the specific event types from google_sign_in package
    if (event is GoogleSignInAuthenticationEventSignIn) {
      googleUser = event.user;
    } else if (event is GoogleSignInAuthenticationEventSignOut) {
      googleUser = null;
    } else if (event is GoogleSignInException) {
      // Handle exception events if necessary, e.g., log the error
      print('AuthService: Authentication event exception: ${event.toString()}');
      googleUser = null; // Treat as sign out or failed sign-in
    }

    if (googleUser != null) {
      print(
        'AuthService: Google user signed in: ${googleUser.email}. Initiating backend login...',
      );
      ...
      try {
        ...
        // Delegate to GoogleClassroomService to handle the backend call and role determination
        ...
        if (loginResult != null &&
          ...
          _jwt = loginResult['token'] as String; // Extract JWT token
  ...
  Future<void> signInWithMoodle({
    ...
    try {
      // MoodleService.login now returns a Map containing 'user' and 'token'
      ...
      if (loginResult.containsKey('user') &&
        ...
        _jwt = loginResult['token'] as String;
...
// Global singleton instance of AuthService for easy access throughout the app.
