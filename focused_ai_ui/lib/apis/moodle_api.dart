...
class MoodleApi {
  ...
  Future<List<dynamic>> listCourses() async {
    print('MoodleApi: Fetching course list...');

    final Uri uri = Uri.parse('${ServerConstants.serverUrl}/moodle/courses');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
          'MoodleApi: Course fetch failed: ${response.statusCode} - $errorMessage',
        );
        throw Exception(
          'Course fetch failed: $errorMessage (Status: ${response.statusCode})',
        );
      }
    } catch (e) {
      print('MoodleApi: Network or parsing error during backend call: $e');
      rethrow; // Re-throw the exception for MoodleService to catch
    }
  }
